# Ignored files

Show files and directories ignored/included by Git, NPM, Yarn, JSR, VSCE or other tools.

See https://github.com/Mopsgamer/view-ignored for details.

![Target selection preivew](https://raw.githubusercontent.com/Mopsgamer/ignored-files/main/images/select-target.png)

![Inversion selection preview](https://raw.githubusercontent.com/Mopsgamer/ignored-files/main/images/select-inversion.png)

## Features

- `Ignored Files: Scan decorations`, `Ignored Files: Clear decorations`

![File tree hover preview](https://raw.githubusercontent.com/Mopsgamer/ignored-files/main/images/hover.png)

- `Explain ignoring reasons`

![File tree context menu selection preview](https://raw.githubusercontent.com/Mopsgamer/ignored-files/main/images/menu-explain.png)

## Context

- `ignoredFiles.isScanning`: Indicates whether the extension is currently scanning for ignored files.
